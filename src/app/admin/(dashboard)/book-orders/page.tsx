...
interface OrderRow { paymentMethod: string; whishPayment: { state: string; submittedAt: string | null } | null; id: string; reference: string; customerName: string; customerPhone: string; purpose: string; requestedBookCount: number; priceCents: number; paymentStatus: string; status: string; isRead: boolean; createdAt: string; }
const statuses = ["NEW", "CONFIRMED", "PREPARING", "READY", "DISPATCHED", "COMPLETED", "CANCELLED"];
const paymentStates = ["AWAITING_PAYMENT", "UNDER_REVIEW", "CHANGES_REQUESTED", "VERIFIED", "EXPIRED", "CANCELLED", "REFUNDED"];
...
export default function BookOrdersPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (paymentState) params.set("paymentState", paymentState); if (paymentMethod) params.set("paymentMethod", paymentMethod);
  ...
  }, [page, purpose, search, status, toast, paymentState, paymentMethod]);
  ...
  const columns: Column<OrderRow>[] = [
    ...
    { key: "paymentMethod", label: "Method", render: (item) => humanize(item.paymentMethod) },
    ...
    { key: "paymentStatus", label: "Payment", render: (item) => <StatusBadge label={humanize(item.whishPayment?.state || item.paymentStatus)} variant={item.paymentStatus === "PAID" ? "success" : "warning"} /> },
  ...
  return <div><AdminPageHeader title="Book Orders" /><a href="/admin/book-whish" className="mb-5 inline-block text-sm font-semibold text-brand-blue underline">Whish payment setup</a><div className="mb-4 flex flex-wrap gap-3"><select aria-label="Filter by status" value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }} className="rounded-xl border border-gray-700/50 bg-[#1e293b] px-3 py-2.5 text-sm text-white"><option value="">All statuses</option>{statuses.map((value) => <option key={value} value={value}>{humanize(value)}</option>)}</select><select aria-label="Filter by purpose" value={purpose} onChange={(event) => { setPurpose(event.target.value); setPage(1); }} className="rounded-xl border border-gray-700/50 bg-[#1e293b] px-3 py-2.5 text-sm text-white"><option value="">All purposes</option>{purposes.map((value) => <option key={value} value={value}>{humanize(value)}</option>)}</select><select aria-label="Filter payment method" value={paymentMethod} onChange={e => { setPaymentMethod(e.target.value); setPage(1); }} className="rounded-xl bg-[#1e293b] px-3 py-2.5 text-sm text-white"><option value="">All payment methods</option><option value="WHISH">Whish</option><option value="CASH_ON_DELIVERY">Cash on delivery / pickup</option><option value="CASH_ARRANGEMENT">Cash arrangement</option></select><select aria-label="Filter Whish verification" value={paymentState} onChange={e => { setPaymentState(e.target.value); setPage(1); }} className="rounded-xl bg-[#1e293b] px-3 py-2.5 text-sm text-white"><option value="">All verification states</option>{paymentStates.map(value => <option key={value} value={value}>{humanize(value)}</option>)}</select></div><AdminDataTable columns={columns} data={data} totalPages={totalPages} currentPage={page} total={total} search={search} onSearch={(value) => { setSearch(value); setPage(1); }} onPageChange={setPage} onView={(item) => router.push(`/admin/book-orders/${item.id}`)} loading={loading} /></div>;
}
