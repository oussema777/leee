...
export default function BookInventoryPage() {
  ...
  const columns: Column<BookInventory>[] = [
    ...
    { key: "sku", label: "SKU" },
    { key: "donor", label: "Donated by", render: (item) => item.donorAllocations.length ? <div className="min-w-40 space-y-1">{item.donorAllocations.map((allocation) => <div key={allocation.donor.displayName} className="flex items-center gap-2">{allocation.donor.logoUrl && allocation.donor.logoApproved && <img src={allocation.donor.logoUrl} alt="" className="size-7 rounded bg-white object-contain p-0.5" />}<span>{allocation.donor.displayName} <span className="text-xs text-gray-500">({allocation.stockQuantity})</span></span></div>)}</div> : item.donor ? <span>{item.donor.displayName}</span> : <span className="text-gray-500">Not recorded</span> },
