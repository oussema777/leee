import { Skeleton } from "@/components/shared/SkeletonBlock";

export default function EntrepreneurLoading() {
  return (
    <>
      {/* Page Header Skeleton */}
      <div className="bg-gradient-to-br from-gray-900 to-gray-800 py-16 px-4 animate-pulse">
        <div className="max-w-7xl mx-auto space-y-3">
          <Skeleton className="h-3 w-56 rounded" />
          <Skeleton className="h-10 w-80 max-w-full rounded" />
          <Skeleton className="h-4 w-96 max-w-full rounded" />
        </div>
      </div>

      {/* Steps Skeleton */}
      <div className="max-w-3xl mx-auto px-4 py-16 space-y-10 animate-pulse">
        <Skeleton className="h-8 w-64 mx-auto rounded mb-4" />
        <Skeleton className="h-4 w-80 mx-auto rounded mb-10" />
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="flex gap-6">
            <Skeleton className="h-14 w-14 rounded-full flex-shrink-0" />
            <div className="flex-1 space-y-2 pt-2">
              <Skeleton className="h-5 w-48 rounded" />
              <Skeleton className="h-4 w-full rounded" />
            </div>
          </div>
        ))}
      </div>

      {/* Program Cards Skeleton */}
      <div className="bg-surface-secondary py-16 px-4 animate-pulse">
        <div className="max-w-4xl mx-auto">
          <Skeleton className="h-8 w-56 mx-auto rounded mb-10" />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {Array.from({ length: 2 }).map((_, i) => (
              <div key={i} className="bg-white rounded-2xl p-8 space-y-3">
                <Skeleton className="h-12 w-12 rounded-full" />
                <Skeleton className="h-6 w-48 rounded" />
                <Skeleton className="h-4 w-full rounded" />
                <Skeleton className="h-4 w-4/5 rounded" />
              </div>
            ))}
          </div>
        </div>
      </div>
    </>
  );
}
