...
const input = {
  ...
  selectedBookIds: ["book-1"], customerName: "Test purchaser", customerPhone: "70123456",
...
beforeEach(() => {
  ...
  Object.assign(config, { enabled: true, accountName: "Test LEE", accountNumber: "70123456", supportPhone: "96170123456" });
...
describe("Whish order creation", () => {
  ...
  it("maps a single-book delivery to the $9 code and rejects unverified setup", async () => {
    ...
    expect(saved.snapshot.amountCents).toBe(900); expect(saved.snapshot.imageUrl).toBe("/payments/whish/usd-9.jpg");
