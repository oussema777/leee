...
const governorateLabels: Record<(typeof LEBANON_GOVERNORATES)[number], readonly [string, string]> = {
    BEIRUT: ["Beirut", "بيروت"],
    ...
    KESERWAN_JBEIL: ["Keserwan-Jbeil", "كسروان جبيل"],
};
...
export function BookDetailExperience({ book, books, locale }: {
    book: CatalogueBook;
    books: CatalogueBook[];
    locale: string;
}) {
    ...
    useEffect(() => {
        ...
        const closeOnEscape = (event: KeyboardEvent) => { if (event.key === "Escape" && !submitting)
            setOpen(false); };
    ...
    const isDeliveryOrder = purpose === "GIFT" || (purpose === "SELF" && fulfillment === "DELIVERY");
    const checkoutDeliveryFee = activePackage && isDeliveryOrder ? deliveryFeeCents(packageKey, "DELIVERY") : 0;
    const checkoutTotal = (activePackage?.priceCents || 0) + checkoutDeliveryFee;
    ...
    function afterPurpose() {
        if (!packageKey || !purpose)
            return;
        if (purpose === "DONATION" && selectionMode === "LEE_CHOICE")
            setStep("checkout");
        else if (BOOK_PACKAGES[packageKey].totalBooks > 1)
            setStep("books");
        else
            setStep("checkout");
    }
    function toggleBook(id: string) {
        ...
        setSelectedIds((current) => {
            ...
            if (current.length >= activePackage.totalBooks)
                return current;
            return [...current, id];
        });
    }
        ...
        <Container className="relative py-8 md:py-12">
          ...
          <div className="grid items-center gap-10 lg:grid-cols-[minmax(320px,0.8fr)_1.2fr] lg:gap-16">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative mx-auto aspect-[4/5] w-full max-w-md overflow-hidden rounded-[2rem] bg-white/10 shadow-2xl ring-1 ring-white/15">
              {book.coverImageUrl ? <Image unoptimized src={book.coverImageUrl} alt={isArabic ? `غلاف كتاب ${title}` : `Cover of ${title}`} fill priority sizes="(max-width: 1024px) 90vw, 420px" className="object-cover"/> : <div className="flex h-full items-center justify-center p-10 text-center">
<BookOpen className="mx-auto mb-5 h-14 w-14 text-brand-blue-light"/>
...
<span className="rounded-full bg-white/10 px-3 py-1.5 text-xs font-semibold text-white/80">{prettyValue(book.condition)}</span>
              ...
              {(book.donorAllocations.length > 0 || book.donor) && <div className="mt-4 flex items-center gap-3 text-sm text-white/70">
                {(() => { const donors = book.donorAllocations.length ? book.donorAllocations.map((allocation) => allocation.donor) : book.donor ? [book.donor] : []; const publicDonors = donors.filter((donor) => donor.publicRecognition); const logo = publicDonors.find((donor) => donor.type === "ORGANISATION" && donor.logoApproved && donor.logoUrl); const names = publicDonors.map((donor) => donor.displayName); return <>{logo?.logoUrl && <Image unoptimized src={logo.logoUrl} alt="" width={36} height={36} className="size-9 rounded-lg bg-white object-contain p-1" />}<span>{isArabic ? "\u0645\u062a\u0628\u0631\u0639 \u0628\u0647 \u0645\u0646" : "Donated by"} <strong className="text-white">{names.length ? names.join(isArabic ? "\u060c " : ", ") : (isArabic ? "\u062f\u0627\u0639\u0645\u064a\u0646 \u0645\u0646 \u0627\u0644\u0645\u062c\u062a\u0645\u0639" : "community supporters")}</strong></span></>; })()}
              </div>}
...
<div>
<Languages className="mb-2 h-4 w-4 text-brand-blue-light"/>
<span className="block text-xs text-white/45">{t.language}</span>
<strong className="mt-1 block">{prettyValue(book.language)}</strong>
</div>
<div>
<Tag className="mb-2 h-4 w-4 text-brand-blue-light"/>
<span className="block text-xs text-white/45">{t.condition}</span>
<strong className="mt-1 block">{prettyValue(book.condition)}</strong>
</div>
<div>
...
<BookHeart className="h-5 w-5"/>{t.order}</button>
...
<Container>
<div className="grid gap-8 rounded-[2rem] bg-white p-7 shadow-sm md:grid-cols-[auto_1fr] md:items-center md:p-10">
<div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-brand-blue-light text-brand-blue-deeper">
<HeartHandshake className="h-8 w-8"/>
</div>
<div>
<h2 className="font-serif text-3xl text-accent-navy">{t.impactTitle}</h2>
<p className="mt-2 max-w-3xl leading-7 text-text-secondary">{t.impactBody}</p>
</div>
</div>
</Container>
</section>

      {related.length > 0 && <section className="pb-16">
<Container>
<h2 className="mb-7 font-serif text-3xl text-accent-navy">{t.related}</h2>
<div className="grid gap-5 sm:grid-cols-3">{related.map((item) => <Link key={item.id} href={`/books/${item.slug}`} className="group flex items-center gap-4 rounded-2xl bg-white p-4 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
<div className="relative h-28 w-20 shrink-0 overflow-hidden rounded-xl bg-surface-secondary">{item.coverImageUrl ? <Image unoptimized src={item.coverImageUrl} alt="" fill sizes="80px" className="object-cover"/> : <BookOpen className="absolute inset-0 m-auto h-7 w-7 text-accent-navy/35"/>}</div>
<div>
<h3 className="font-serif text-lg leading-snug text-text-primary group-hover:text-brand-blue-deeper">{localTitle(item, isArabic)}</h3>
<p className="mt-2 text-xs text-text-muted">{localAuthor(item, isArabic)}</p>
</div>
</Link>)}</div>
</Container>
</section>}
      ...
      <AnimatePresence>
        {open && <motion.div className="fixed inset-0 z-[100] flex items-end justify-center bg-accent-navy/70 p-0 backdrop-blur-sm md:items-center md:p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget && !submitting)
            setOpen(false); }}>
          <motion.section role="dialog" aria-modal="true" aria-label={t.checkoutTitle} className="flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-t-[2rem] bg-accent-ice shadow-2xl md:rounded-[2rem]" initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }} transition={{ type: "spring", stiffness: 320, damping: 30 }}>
            <header className="flex items-center justify-between border-b border-surface-tertiary bg-white px-5 py-4 md:px-7">
<div className="flex items-center gap-4">
<div className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-blue-light text-brand-blue-deeper">
<BookHeart className="h-5 w-5"/>
</div>
<div>
<p className="text-xs font-bold uppercase tracking-[0.16em] text-brand-blue-deeper">LEE Book Restore</p>{step !== "success" && <div className="mt-1 flex items-center gap-1.5" aria-label={`Step ${stepNumber} of 4`}>{[1, 2, 3, 4].map((number) => <span key={number} className={`h-1.5 rounded-full transition-all ${number <= stepNumber ? "w-7 bg-brand-blue" : "w-3 bg-surface-tertiary"}`}/>)}</div>}</div>
</div>
<button type="button" onClick={() => setOpen(false)} disabled={submitting} aria-label={t.close} className="rounded-full p-2 text-text-secondary transition hover:bg-surface-secondary hover:text-text-primary">
<X className="h-5 w-5"/>
</button>
</header>
            ...
            <div className="overflow-y-auto p-5 md:p-8">
              <AnimatePresence mode="wait" initial={false}>
                {step === "package" && <motion.div key="package" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
<h2 className="font-serif text-3xl text-accent-navy md:text-4xl">{t.packageTitle}</h2>
<p className="mt-3 max-w-2xl text-sm leading-6 text-text-secondary">{t.packageIntro}</p>
<div className="mt-7 grid gap-4 sm:grid-cols-2">{packageKeys.map((key) => { const details = BOOK_PACKAGES[key]; const labels = packageCopy[key]; const active = packageKey === key; return <button type="button" key={key} onClick={() => { setPackageKey(key); setSelectedIds([book.id]); }} className={`relative rounded-2xl border-2 p-5 text-start transition-all hover:-translate-y-0.5 hover:shadow-lg ${active ? "border-brand-blue bg-white shadow-lg shadow-brand-blue/10" : "border-transparent bg-white hover:border-brand-blue/30"}`}>
<div className="flex items-start justify-between gap-4">
<div>
<p className="font-serif text-2xl text-accent-navy">{t[labels[0]]}</p>
<p className="mt-2 text-sm leading-6 text-text-secondary">{t[labels[1]]}</p>
</div>
<span className="rounded-xl bg-accent-navy px-3 py-2 text-lg font-bold text-white">${details.priceCents / 100}</span>
</div>{details.freeBooks > 0 && <span className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-amber-100 px-3 py-1 text-xs font-bold text-amber-900">
<Sparkles className="h-3.5 w-3.5"/>{details.freeBooks} {t.freeExtra}</span>}{active && <motion.span layoutId="package-check" className="absolute -end-2 -top-2 flex h-7 w-7 items-center justify-center rounded-full bg-brand-blue text-white">
<Check className="h-4 w-4"/>
</motion.span>}</button>; })}</div>
<div className="mt-7 flex justify-end">
<button type="button" disabled={!packageKey} onClick={() => setStep("purpose")} className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-accent-navy px-6 py-3 font-bold text-white transition hover:bg-brand-blue-deeper disabled:opacity-40">{t.continue}{isArabic ? <ChevronLeft className="h-4 w-4"/> : <ArrowRight className="h-4 w-4"/>}</button>
</div>
</motion.div>}

                {step === "purpose" && <motion.div key="purpose" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
<h2 className="font-serif text-3xl text-accent-navy md:text-4xl">{t.purposeTitle}</h2>
<p className="mt-3 text-sm text-text-secondary">{t.purposeIntro}</p>
<div className="mt-7 grid gap-4 md:grid-cols-3">{([{ key: "SELF", icon: Home, title: t.selfTitle, body: t.selfBody }, { key: "GIFT", icon: Gift, title: t.giftTitle, body: t.giftBody }, { key: "DONATION", icon: HandHeart, title: t.donateTitle, body: t.donateBody }] as const).map((option) => { const active = purpose === option.key; return <motion.button whileHover={{ y: -4 }} whileTap={{ scale: .98 }} type="button" key={option.key} onClick={() => { setPurpose(option.key); setSelectionMode(option.key === "DONATION" ? "LEE_CHOICE" : "CUSTOM"); }} className={`rounded-2xl border-2 bg-white p-5 text-start transition-colors ${active ? "border-brand-blue shadow-lg shadow-brand-blue/10" : "border-transparent hover:border-brand-blue/30"}`}>
<motion.span animate={active ? { rotate: [0, -7, 7, 0], scale: [1, 1.12, 1] } : {}} className={`flex h-14 w-14 items-center justify-center rounded-2xl ${active ? "bg-brand-blue text-white" : "bg-brand-blue-light text-brand-blue-deeper"}`}>
<option.icon className="h-7 w-7"/>
</motion.span>
<h3 className="mt-5 font-serif text-xl text-accent-navy">{option.title}</h3>
<p className="mt-2 text-sm leading-6 text-text-secondary">{option.body}</p>{active && <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="mt-4 flex items-center gap-2 text-xs font-bold text-brand-blue-deeper">
<CheckCircle2 className="h-4 w-4"/>{t.selected}</motion.div>}</motion.button>; })}</div>{purpose === "DONATION" && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mt-5 grid gap-3 rounded-2xl bg-white p-4 sm:grid-cols-2">
<button type="button" onClick={() => setSelectionMode("LEE_CHOICE")} className={`rounded-xl border-2 p-4 text-start ${selectionMode === "LEE_CHOICE" ? "border-brand-blue bg-brand-blue-light/50" : "border-surface-tertiary"}`}>
<strong className="flex items-center gap-2 text-sm text-accent-navy">
<Sparkles className="h-4 w-4 text-brand-blue"/>{t.leeChoose}</strong>
<span className="mt-1 block text-xs text-text-secondary">{t.leeChooseHint}</span>
</button>
<button type="button" onClick={() => setSelectionMode("CUSTOM")} className={`rounded-xl border-2 p-4 text-start text-sm font-bold text-accent-navy ${selectionMode === "CUSTOM" ? "border-brand-blue bg-brand-blue-light/50" : "border-surface-tertiary"}`}>{t.chooseMyself}</button>
</motion.div>}<div className="mt-7 flex items-center justify-between">
<button type="button" onClick={() => setStep("package")} className="px-3 py-2 text-sm font-bold text-text-secondary">{t.back}</button>
<button type="button" disabled={!purpose} onClick={afterPurpose} className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-accent-navy px-6 py-3 font-bold text-white transition hover:bg-brand-blue-deeper disabled:opacity-40">{t.continue}{isArabic ? <ChevronLeft className="h-4 w-4"/> : <ArrowRight className="h-4 w-4"/>}</button>
</div>
</motion.div>}

                {step === "books" && activePackage && <motion.div key="books" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
<div className="flex flex-wrap items-end justify-between gap-4">
<div>
<h2 className="font-serif text-3xl text-accent-navy md:text-4xl">{t.booksTitle}</h2>
<p className="mt-2 text-sm font-bold text-brand-blue-deeper" aria-live="polite">{t.booksProgress(selectedIds.length, activePackage.totalBooks)} · {remaining ? t.booksRemaining(remaining) : t.full}</p>
</div>
<div className="min-w-40 rounded-xl bg-accent-navy px-4 py-3 text-center text-white">
<span className="text-2xl font-bold tabular-nums">{selectedIds.length}/{activePackage.totalBooks}</span>
</div>
</div>
<div className="mt-5 flex gap-2 overflow-x-auto pb-2">{selectedBooks.map((item, index) => <motion.div layout key={item.id} className="relative h-24 w-16 shrink-0 overflow-hidden rounded-lg bg-surface-secondary ring-2 ring-white shadow">
<button type="button" onClick={() => toggleBook(item.id)} aria-label={`${t.remove} ${localTitle(item, isArabic)}`} className="absolute end-1 top-1 z-10 rounded-full bg-accent-navy/90 p-1 text-white">
<X className="h-3 w-3"/>
</button>{item.coverImageUrl ? <Image unoptimized src={item.coverImageUrl} alt="" fill sizes="64px" className="object-cover"/> : <BookOpen className="absolute inset-0 m-auto h-5 w-5 text-accent-navy/30"/>}{activePackage.freeBooks > 0 && index >= activePackage.paidBooks && <span className="absolute bottom-0 inset-x-0 bg-amber-400 px-1 py-0.5 text-center text-[8px] font-bold text-amber-950">{t.freeExtra}</span>}</motion.div>)}</div>
<label className="relative mt-5 block">
<Search className="pointer-events-none absolute start-4 top-1/2 h-4 w-4 -translate-y-1/2 text-text-muted"/>
<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t.search} className={`${fieldClass} mt-0 ps-11`}/>
</label>
<div className="mt-5 grid max-h-[42vh] gap-3 overflow-y-auto pe-1 sm:grid-cols-2 lg:grid-cols-3">{filteredBooks.map((item) => { const active = selectedIds.includes(item.id); const disabled = !active && remaining === 0; return <button type="button" key={item.id} disabled={disabled} onClick={() => toggleBook(item.id)} className={`flex items-center gap-3 rounded-2xl border-2 bg-white p-3 text-start transition ${active ? "border-brand-blue shadow-sm" : "border-transparent hover:border-brand-blue/30 disabled:opacity-45"}`}>
...
<button type="button" onClick={() => setStep("purpose")} className="px-3 py-2 text-sm font-bold text-text-secondary">{t.back}</button>
<button type="button" disabled={remaining !== 0} onClick={() => setStep("checkout")} className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-accent-navy px-6 py-3 font-bold text-white transition hover:bg-brand-blue-deeper disabled:opacity-40">{t.continue}{isArabic ? <ChevronLeft className="h-4 w-4"/> : <ArrowRight className="h-4 w-4"/>}</button>
</div>
</motion.div>}
...
</section>{purpose === "SELF" && <section className="rounded-2xl bg-white p-5">
<h3 className="font-serif text-xl text-accent-navy">{t.fulfillment}</h3>
<div className="mt-4 grid grid-cols-2 gap-3">{([{ key: "DELIVERY", icon: Truck, label: t.delivery }, { key: "PICKUP", icon: MapPin, label: t.pickup }] as const).map((option) => <button type="button" key={option.key} onClick={() => setFulfillment(option.key)} className={`rounded-xl border-2 p-4 text-start text-sm font-bold ${fulfillment === option.key ? "border-brand-blue bg-brand-blue-light/50 text-accent-navy" : "border-surface-tertiary text-text-secondary"}`}>
<option.icon className="mb-2 h-5 w-5"/>{option.label}</button>)}</div>
</section>}{purpose === "GIFT" && <section className="rounded-2xl bg-white p-5">
<h3 className="flex items-center gap-2 font-serif text-xl text-accent-navy">
...
<div className="mt-5 flex items-end justify-between border-b border-white/15 pb-5">
<div>
<p className="text-sm font-bold">{t[packageCopy[packageKey][0]]}</p>
<p className="mt-1 text-xs text-white/55">{purpose === "SELF" ? t.selfTitle : purpose === "GIFT" ? t.giftTitle : t.donateTitle}</p>
</div>
<strong className="text-2xl">${activePackage.priceCents / 100}</strong>
</div>
{isDeliveryOrder && <div className="flex items-center justify-between border-b border-white/15 py-4 text-sm">
<span className="text-white/70">{t.deliveryFee}</span>
{checkoutDeliveryFee > 0 ? <strong>${checkoutDeliveryFee / 100}</strong> : <span className="flex items-center gap-2"><span className="text-white/45 line-through">${DELIVERY_FEE_CENTS / 100}</span><strong className="text-emerald-300">{t.free}</strong></span>}
</div>}
<div className="flex items-center justify-between border-b border-white/15 py-4">
<strong>{t.total}</strong>
<strong className="text-2xl">${checkoutTotal / 100}</strong>
</div>
...
<button type="button" onClick={() => setOpen(false)} className="mt-7 inline-flex min-h-12 items-center gap-2 rounded-xl bg-accent-navy px-6 py-3 font-bold text-white hover:bg-brand-blue-deeper">{t.keepBrowsing}<BookOpen className="h-4 w-4"/>
</button>
</motion.div>}
              </AnimatePresence>
            </div>
